...
var TimeGridEventRenderer = /** @class */ (function (_super) {
    ...
    //
    // The segment might be part of a "serials", which means consecutive segments with the same pressure
    // who's width is unknown until an edge has been hit. `serialsBackwardPressure` is the number of
    // segments behind this one in the current serials, and `serialsBackwardCoord` is the starting
    // coordinate of the first segment in the serials.
    TimeGridEventRenderer.prototype.computeFgSegForwardBack = function (seg, serialsBackwardPressure, serialsBackwardCoord) {
        ...
        if (seg.forwardCoord === undefined) { // not already computed
            if (!forwardSegs.length) {
                ...
                this.computeFgSegForwardBack(forwardSegs[0], serialsBackwardPressure + 1, serialsBackwardCoord);
                seg.forwardCoord = forwardSegs[0].backwardCoord;
            }
            // calculate the backwardCoord from the forwardCoord. consider the serials
            seg.backwardCoord = seg.forwardCoord -
                (seg.forwardCoord - serialsBackwardCoord) / // available width for serials
                    (serialsBackwardPressure + 1); // # of segments in the serials
